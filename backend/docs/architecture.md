...
- `capture_weight` возвращает результат через status topic как `commandStatus` с `commandId` и `result.raw_weight_kg`; ingestion-service сохраняет этот результат в `device_commands.result` и переводит команду в `acknowledged` или `failed`.
- после `finish_config_session` или истечения hold timeout firmware снова разрешает deep sleep, предварительно отправив `commandStatus`.
- `worker-service` переводит `created` / `published` команды в `expired`, если `expires_at` уже прошел и подтверждение от устройства не пришло.
