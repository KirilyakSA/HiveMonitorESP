...
import (
	...
	"github.com/KirilyakSA/HiveMonitorESP/backend/internal/worker"
)
...
func main() {
	...
	service := worker.NewService(cfg, repo, bus, logger)
	if err := service.Run(ctx); err != nil {
		logger.Error("worker service stopped", "error", err)
		os.Exit(1)
	}
}
